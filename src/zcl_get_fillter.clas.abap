CLASS zcl_get_fillter DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-DATA:
     "Instance Singleton
     mo_instance      TYPE REF TO zcl_get_fillter.

    CLASS-METHODS:
      "Contructor khỏi tạo đối tượng
      get_Instance RETURNING VALUE(ro_instance) TYPE REF TO zcl_get_fillter,
...
CLASS ZCL_GET_FILLTER IMPLEMENTATION.
  ...
  METHOD get_fillter_app.
    ...
    LOOP AT lr_ranges INTO DATA(ls_ranges).
      CASE ls_ranges-name.
        ...
        WHEN 'RLDNR'.
          MOVE-CORRESPONDING ls_ranges-range TO ir_rldnr.
        WHEN 'GJAHR'.
